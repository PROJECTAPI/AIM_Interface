        ...
        public static string UpdateLog()
        {
            ...
            try
            {
                using (OracleConnection connection = new OracleConnection())
                {
                    ...
                    connection.Open();
                   // var commandText = new object();

                    DataTable commandText = null;

                    if (PropertiesModel.IsMainAPiLog)
                    {
                        commandText = Services.GetDataTable("Select * from APILOG Where  PK_APILOG = " + PropertiesModel.MainPK_APILog + "");
                    }
                    else
                    {
                        commandText = Services.GetDataTable("Select * from APILOG Where PK_APILOG = " + PropertiesModel.LastPK_APILOGInserted + "");
                    }
                    ...
                    for (int i = 0; i < rows.Length; i++)
                    {
                        ...
                        rows[i]["TRANS_LOG"] = PropertiesModel.LogDetailsMessage;
                        rows[i]["API_SUCCES"] = PropertiesModel.LogDetailsStatus;
                        rows[i]["OP_UPDATE"] = "APIBATCH";
                        rows[i]["D_UPDATE"] = DateTime.Now.ToString("dd/MMM/yyyy");
                        rows[i]["T_UPDATE"] = DateTime.Now.ToLocalTime().ToString("h:mm:ss");
                    }
